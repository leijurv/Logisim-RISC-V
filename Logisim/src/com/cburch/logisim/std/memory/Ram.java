...
//BEGIN MODIFIED SECTION
                int tmp = dataValue.toIntValue();
                MemContents contents = myState.getContents();
                //System.out.println("Writing data value " + tmp + " to " + addr);
                if (contents.getLogLength() == 24 && contents.getWidth() == 32) {
                    byte[] toStore = encode(tmp);
                    for (int i = 0; i < 4; i++) {
        ...
        if (outputEnabled) {
            ...
            int val;
            if (contents.getLogLength() == 24 && contents.getWidth() == 32) {
                byte[] toStore = new byte[4];
                for (int i = 0; i < 4; i++) {
                    //write toStore[i] to addr+i
                    int loc = addr + i;

                    int val2 = contents.get(loc / 4);
